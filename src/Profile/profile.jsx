...
function Profile() {
  ...
  // Listening to "profile" collection
  useEffect(() => {
  ...
  const isDuplicateUser = (data, excludeIndex = null) =>
    users.some(
      (user, i) =>
        i !== excludeIndex &&
        user.name.trim().toLowerCase() === data.name.trim().toLowerCase() && 
  ...
  const handleAddUser = async () => {
    ...
    try {
      if (editingIndex !== null) {
        ...
        await updateDoc(doc(db, "profile", userToEdit.id), userData); // "profile" collection
        setEditingIndex(null);
        showToast({ type: "success", message: `${fullName}'s changes were saved successfully.` });
      } else {
        await addDoc(collection(db, "profile"), userData); // "profile" collection
        showToast({ type: "success", message: `${fullName} was added successfully.` });
      }

      setFormData({ name: "", surname: "", email: "", department: "", role: "" });
      setEmailError("");
      setDuplicateError("");
    } catch (error) {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      await deleteDoc(doc(db, "profile", deletedUser.id));
  ...
  const handleUndo = async () => {
    ...
    const { id, ...userData } = toast.deletedUser;
    try {
      await addDoc(collection(db, "profile"), userData);
      setToast(null);
    } catch (error) {
      console.error("Error restoring user:", error);
      alert("Could not undo deletion. Please try again.");
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
          ...
          <table className="min-w-[820px] w-full border border-gray-300">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="text-center text-gray-400 py-6 text-sm">Loading profiles...</td>
                </tr>
              ) : users.length === 0 ? (
                <tr>
                  ...
                  <tr key={user.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                    ...
                    <td className="p-2 border border-gray-300">
                      <div className="flex items-center justify-center gap-3">
                        ...
                        <button type="button" onClick={() => handleDelete(index)} title="Delete"
      ...
      {toast && (
        <div className="fixed bottom-4 left-1/2 z-50 flex w-[calc(100%-2rem)] max-w-md -translate-x-1/2 items-center gap-4 rounded-xl bg-teal-700 px-4 py-3 text-white shadow-xl animate-fade-in-up sm:bottom-6 sm:w-auto sm:px-5">
          ...
          <span className="text-sm">
            {toast.type === "success"
              ? toast.message
              : <><span className="font-semibold">{toast.deletedUser?.name} {toast.deletedUser?.surname}</span> was deleted.</>
            }
          </span>
          {toast.type === "deleted" && (
            <button type="button" onClick={handleUndo}
              className="text-white hover:text-teal-200 text-sm font-bold underline underline-offset-2 transition-colors">
              Undo
            </button>
          )}
          ...
          <div className="absolute bottom-0 left-0 h-1 bg-teal-300 rounded-b-xl animate-shrink-bar"
